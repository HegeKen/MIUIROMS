...
//当前受支持的小米机型代号；
$fb = array("grus","umi","cmi","cas","venus","star");
$sb = array("haydn","alioth","thyme","lmi","picasso","phoenix","cezanne","apollo","vangogh","picasso_48m","atom","bomb","tucana","pyxis","crux","cepheus","raphael","vela","begonia","ares","renior","cetus","lime","cannon","gauguin");
...
function fastbootROM($device)
{
    $deviceName = array(
        ...
        "lime" => "Redmi Note 9 4G",
        "cannon" => "Redmi Note 9 5G",
        "gauguin" => "Redmi Note 9 Pro",
    ...
    $device = $device;
    $url = "http://update.miui.com/updates/v1/fullromdownload.php?d=" . $device . "&b=F&r=cn";
    ...
    //var_dump($result);
    preg_match_all('/bigota.*?tgz/m', $result, $match);
    if ($match[0] == null) {
        //echo "Error @".$device."=>".$deviceName[$device].$url."</br>";
    } else {
        $romdata = explode("/",$match[0][0]);
        //var_dump($romdata);
        $ver = $romdata[1];
        echo $deviceName[$device] . "&nbsp;".$ver."&nbsp;线刷包：https://" . $match[0][0] . "</br>";
    }
}

echo " </br>";
echo "<p>首批MIUI12.5机型：</p>";
echo " </br>";
foreach ($fb as $device) {
    fastbootROM($device);
}
echo " </br>";
echo "<p>第二批MIUI12.5机型：</p>";
echo " </br>";
foreach ($sb as $device) {
    fastbootROM($device);
}