...
echo "<table><thead><tr><td>机型</td><td>检测到版本</td><td>最新收录的版本</td><td>链接</td></tr></thead><tbody>";
    foreach ($gbDevices as $gbDevices) {
        $gbDevices = $gbDevices;
        $url = "http://update.miui.com/updates/v1/fullromdownload.php?d=" . $gbDevices;
        ...
        if ($match[0] == null) {
            ...
            if($cversion == $gbCurrent[$gbDevices]){
                //echo "<tr><td>" .$deviceName[$cnDevices] . "</td><td>". $cversion ."</td><td>" . $gbCurrent[$gbDevices] ."</td><td>未发现更新</td></tr>";
            }else{
                echo "<tr><td>" .$deviceName[$gbDevices] . "</td><td>" . $cversion ."</td><td>" . $gbCurrent[$gbDevices] ."</td><td>" . $clink ."</td></tr>";
            }
        }
    };
    echo "</tbody></table>";